...
differential_nichenet = function(i, path_seurat_obj){
  ...
  geneset_MoMac2 = DE_receiver_processed_targets %>% filter(receiver == "MoMac2" & target_score >= lfc_cutoff & target_significant == 1 & target_present == 1) %>% pull(target) %>% unique()
...
saveRDS(job_liver_de, "output/job_differential_nichenet_mouse_MoMac2.rds")

output = readRDS("output/job_differential_nichenet_mouse_MoMac2.rds")

output = qsub_retrieve(output)

saveRDS(output %>% .[[1]], "output/differential_nichenet_mouse_KC_MoMac2_new.rds")
