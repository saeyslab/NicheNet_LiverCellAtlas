...
differential_nichenet = function(i, path_seurat_obj){
  ...
  niches = list(
    "KC_niche" = list(
      ...
      "receiver" = c("KCs")),
    "MoMac1_CV_niche" = list(
      "sender" = c("Central Vein Endothelial cells","Fibroblast 1"),
      "receiver" = c("MoMac1"))
  )
...
saveRDS(job_liver_de, "output/job_differential_nichenet_mouse_MoMac1_CV.rds")

output = readRDS("output/job_differential_nichenet_mouse_MoMac1_CV.rds")

output = qsub_retrieve(output)

saveRDS(output %>% .[[1]], "output/differential_nichenet_mouse_KC_MoMac1_CV.rds")
