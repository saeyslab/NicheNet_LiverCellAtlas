...
#   return(y)
# }
get_prioritization_tables_nozonation = function(output_nichenet_analysis, prioritizing_weights){
...
# 
#         print(DE_subtable)
#         print("done for sender")
# 
#       }, seurat_obj, senders_other) %>% bind_rows()
#       print("done for senders of this niche")
# 
#     }, seurat_obj) %>% bind_rows()
#     print("done for senders of all niches")
# 
#     return(DE_sender)
# 
#   }
...
process_niche_de = function(DE_table, niches, type){
...
process_receiver_target_de = function(DE_receiver_targets, niches, specificity_score = "min_lfc"){
...
process_spatial_de = function(DE_table, type, lr_network, specificity_score = "lfc"){
  ### process the DE_tables of senders and receiver
...
process_spatial_de_receiver_targets = function(DE_table, specificity_score = "lfc"){
  ### process the DE_tables of senders and receiver
  

    DE_receiver = DE_table %>% mutate(significant = p_val_adj <= 0.05, present = pct.1 >= expression_pct) %>% mutate(pct.1 = pct.1+0.0001, pct.2 = pct.2 + 0.0001) %>% mutate(diff = (pct.1/pct.2)) %>% mutate(score = diff*avg_log2FC) 
    if(specificity_score == "lfc"){
      DE_receiver = DE_receiver %>% rename(target = gene, target_score_zonation = avg_log2FC, receiver = celltype) 
    }
    if(specificity_score == "score"){
      DE_receiver = DE_receiver %>% rename(target = gene, target_score_zonation = score, receiver = celltype) 
    }
    DE_receiver = DE_receiver %>% select(niche, receiver, target, target_score_zonation, significant, present)  %>% arrange(-target_score_zonation)
    return(DE_receiver)
