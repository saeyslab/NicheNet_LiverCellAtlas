...
path_seurat_obj = "/group/irc/personal/robinb/Liver_Visium/output/seurat_obj_scrnaseq_human.rds"
...
differential_nichenet = function(i, path_seurat_obj){
  ...
  seurat_obj@meta.data$celltype[seurat_obj@meta.data$celltype == "LAMs"] = "MF"
  seurat_obj@meta.data$celltype[seurat_obj@meta.data$celltype == "MoMac1"] = "MF"
  
  niches = list(
    "KC_niche" = list(
      "sender" = c("LSECs","Hepatocytes","Stellate cells"),
      "receiver" = c("resKCs")),
    "MF_niche" = list(
      "sender" = c("Cholangiocytes","Fibroblasts","Central Vein Endothelial cells"),
      "receiver" = c("MF"))
  )
  ...
  geneset_KC = DE_receiver_processed_targets %>% filter(receiver == "resKCs" & target_score >= lfc_cutoff & target_significant == 1 & target_present == 1) %>% pull(target) %>% unique()
  geneset_MF = DE_receiver_processed_targets %>% filter(receiver == "MF" & target_score >= lfc_cutoff & target_significant == 1 & target_present == 1) %>% pull(target) %>% unique()
  ...
  geneset_MF %>% setdiff(rownames(ligand_target_matrix))
  ...
  return(list(DE_sender_receiver = DE_sender_receiver, ligand_scaled_receptor_expression_fraction_df = ligand_scaled_receptor_expression_fraction_df, 
...
saveRDS(job_liver_de, "output/job_liver_diffNicheNet_human_vs_ALL_new_noPV_EC.rds")


output = readRDS("output/job_liver_diffNicheNet_human_vs_ALL_new_noPV_EC.rds")
output = qsub_retrieve(output)

saveRDS(output %>% .[[1]], "output/differential_nichenet_human_KC_MF_noPV_EC.rds")
