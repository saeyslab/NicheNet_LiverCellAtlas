...
differential_nichenet = function(i, path_seurat_obj){
  ...
  seurat_obj@meta.data$celltype[seurat_obj@meta.data$celltype == "MoMac2"] = "MF"
  ...
  niches = list(
    "KC_niche" = list(
      ...
      "receiver" = c("KCs")),
    "MF_niche" = list(
      "sender" = c("Cholangiocytes","Fibroblast 2", "Central Vein Endothelial cells","Fibroblast 1", "Capsule fibroblasts","Mesothelial cells"),
      "receiver" = c("MF"))
  )
...
saveRDS(job_liver_de, "output/job_differential_nichenet_mouse_MF.rds")

output = readRDS("output/job_differential_nichenet_mouse_MF.rds")

output = qsub_retrieve(output)

saveRDS(output %>% .[[1]], "output/differential_nichenet_mouse_KC_MF_new.rds")
